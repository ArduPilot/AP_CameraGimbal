...
{
    ot_pic_buf_attr attr = {0};
    ot_vb_calc_cfg calc = {0};
...
td_s32 ca_mt11_thermal_send_yuyv(const td_u8 *yuyv)
{
    ...
    mapping = physical != 0 ? ss_mpi_sys_mmap(physical, calc.vb_size) : NULL;
    if (mapping == NULL) {
    ...
    frame.video_frame.virt_addr[1] = frame.video_frame.virt_addr[0] +
                                      calc.main_y_size;
    ...
    if (atomic_load(&output_inverted)) {
        ca_mt11_yuyv_to_nv12(
            ...
            frame.video_frame.virt_addr[0], calc.main_stride,
            frame.video_frame.virt_addr[1], calc.main_stride);
    } else {
