...
{
    void *(*map)(AX_U64,AX_U32)=(void* (*)(AX_U64, AX_U32))(dlsym(RTLD_DEFAULT,"AX_SYS_Mmap"));
    AX_S32 (*unmap)(void *,AX_U32)=(AX_S32 (*)(void*, AX_U32))(dlsym(RTLD_DEFAULT,"AX_SYS_Munmap"));
    ...
    unsigned char *y=(unsigned char*)(map(frame->u64PhyAddr[0]+(AX_U64)b->y*stride+b->x,y_size));
    if (!y || y==(void *)-1) return EIO;
    unsigned char *uv=(unsigned char*)(map(uv_base+(AX_U64)(b->y/2)*uv_stride+b->x,uv_size));
                            ...
                            const struct ca_overlay_bitmap *cross, unsigned request)
{
    ...
    if (request!=overlay_acked && atomic_load(&overlay_done[0])==request &&
        ...
        struct ca_z1_native_header h={CA_Z1_NATIVE_OVERLAY_MAGIC,0,
            ...
            (uint32_t)(atomic_load(&overlay_error)),request&1};
...
int main(int argc, char **argv) {
    ...
    {
    unsigned char floorplan[16384] __attribute__((aligned(8))) = {};
    ...
    void **sensor = (void**)(sym("gSnsimx415Obj"));
    ...
    for (unsigned c = 0; c < 2; c++) {
        ...
        attr.stRcAttr.stH264Cbr = (AX_VENC_H264_CBR_S){.u32Gop = 30,
                                                       ...
                                                       .u32SrcFrameRate = 30,
                                                       .fr32DstFrameRate = 30,
                                                       .u32BitRate = (AX_U32)(c ? 24000 : 4000),
                                                       .u32MaxQp = 51,
                                                       .u32MinQp = 20,
                                                       .u32MaxIQp = 51,
                                                       .u32MinIQp = 20,
    ...
    struct encoder_worker workers[2] = {};
