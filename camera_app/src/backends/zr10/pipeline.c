...
#define LOAD(lib,name,args) mi.name=dlsym(RTLD_DEFAULT,#name); \
    if(!mi.name) { ca_log("missing %s",#name);errno=ENOENT;return -1; }
    MI_FUNCTIONS(LOAD)
    get_af_stats=dlsym(RTLD_DEFAULT,"MI_ISP_CUS3A_GetAFStats");
...
static int sensor_start(void)
{
    ...
    i6_snr_pad pad={0};i6_snr_plane plane={0};
    ...
    i6_common_pixfmt raw=I6_PIXFMT_RGB_BAYER+plane.precision*I6_BAYER_END+plane.bayer;
    ...
    i6_vif_port vp={.capt=plane.capt,.dest={2560,1440},.field=3,.pixFmt=raw,
        .frate=I6_VIF_FRATE_FULL,.frameLineCnt=1440};
    CHECK(mi.MI_VIF_SetChnPortAttr(0,0,&vp));
    CHECK(mi.MI_VIF_EnableChnPort(0,0));vif_port=true;
    i6_vpe_chn vc={.capt={2560,1440},.pixFmt=raw,.hdr=I6_HDR_OFF,
        .sensor=I6_VPE_SENS_ID0,.mode=I6_VPE_MODE_REALTIME};
    CHECK(mi.MI_VPE_CreateChannel(0,&vc));vpe_created=true;
    i6_vpe_para param={.hdr=I6_HDR_OFF,.level3DNR=1};
...
{
    i6_vpe_port port={.output={s->width,s->height},.pixFmt=I6_PIXFMT_YUV420SP,
        .mirror=inverted,.flip=inverted};
    ...
    if(jpeg) {
        attr.attrib.mjpg=(i6_venc_attr_mjpg){.maxWidth=s->width,.maxHeight=s->height,
            .width=s->width,.height=s->height,.bufSize=s->width*s->height*2,.byFrame=1};
        ...
        attr.attrib.h264=(i6_venc_attr_h26x){.maxWidth=s->width,.maxHeight=s->height,
            .width=s->width,.height=s->height,.bufSize=s->width*s->height,
            .byFrame=1,.profile=s->codec==CA_VIDEO_H264 ? 1:0};
...
int ca_zr10_venc_get(unsigned ch,uint8_t **data,size_t *length,uint64_t *pts_us)
{
    ...
    *data=NULL;*length=0;
    i6_venc_stat stat={0};CHECK(mi.MI_VENC_Query(ch,&stat));
    if(!stat.curPacks)return 0;
    if(stat.curPacks>256) { errno=EPROTO;return -1; }
    i6_venc_pack *packs=calloc(stat.curPacks,sizeof(*packs));
    if(!packs)return -1;
    ...
    size_t total=0;uint8_t *buffer=NULL;
    ...
    buffer=malloc(total);if(!buffer)goto release;
    size_t offset=0;
    for(unsigned i=0;i<stream.count;i++) {
...
int ca_zr10_set_inverted(bool value)
{
    ...
    for(unsigned i=0;i<CA_ZR10_VENC_COUNT;i++) {
        i6_vpe_port port={.output={cfg.streams[i].width,cfg.streams[i].height},
            .pixFmt=I6_PIXFMT_YUV420SP,.mirror=value,.flip=value};
        ...
        if(r) {
            ...
            for(unsigned j=0;j<i;j++) {
                i6_vpe_port old={.output={cfg.streams[j].width,cfg.streams[j].height},
                    .pixFmt=I6_PIXFMT_YUV420SP,.mirror=inverted,.flip=inverted};
