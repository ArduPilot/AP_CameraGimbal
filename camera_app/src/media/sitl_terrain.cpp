...
static int transfer(int fd, void *buffer, size_t length, bool writing)
{
    ...
    while (length) {
        struct pollfd f = {.fd = fd, .events = (short int)(writing ? POLLOUT : POLLIN)};
                         ...
                         const enum ca_video_codec codecs[CA_SITL_STREAMS])
{
...
#endif
    struct ca_sitl_terrain *t = (struct ca_sitl_terrain*)(calloc(1, sizeof(*t)));
    ...
    const char *argv[] = {(char *)python, (char *)script, (char *)link_option, (char *)link_value, "--width1", w1,
...
#endif
        if (!code) code = posix_spawnp(&t->pid, python, &actions, NULL, const_cast<char *const *>(argv), environ);
                          ...
                          uint8_t *photos[3], size_t photo_length[3], struct ca_exposure *exposure, struct ca_sitl_raw_thermal *raw)
{
    ...
    for (unsigned i = 0; i < CA_SITL_STREAMS; i++) {
        ...
        data[i] = (uint8_t*)(malloc(length[i]));
    ...
    for (unsigned i = 0; i < 3; i++) {
        ...
        photos[i] = (uint8_t*)(malloc(photo_length[i]));
