...
{
    struct ca_support_mavlink *proxy = opaque;
    int fd = -1;
    unsigned retry = 0;
    while (!atomic_load(&proxy->stop)) {
        ...
        for (unsigned i = 0; i < QUEUE_SIZE; i++) {
            ...
            mavlink_status_t status = {.current_tx_seq = message.seq};
                            ...
                            const struct ca_support_config *config)
{
    *result = NULL;
    ...
    struct ca_support_mavlink *proxy = calloc(1, sizeof(*proxy));
    if (!proxy) return -1;
    proxy->config = *config;
    ...
    if (error) { free(proxy); errno = error; return -1; }
    error = pthread_create(&proxy->thread, NULL, relay, proxy);
    if (error) { pthread_mutex_destroy(&proxy->lock); free(proxy); errno = error; return -1; }
    *result = proxy;
    return 0;
}
