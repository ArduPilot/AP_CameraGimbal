...
_Static_assert(sizeof(struct fmt_record)==89, "DataFlash FMT layout");
...
{
    if (size>252) return;
    struct entry entry={.kind=0,.length=(uint16_t)(size+3)};
...
{
    struct ca_log_parm r={.time_us=ca_binlog_time_us(),.value=value};
...
int ca_binlog_init(const char *root)
{
    ...
    logger.queue=calloc(QUEUE_SIZE,sizeof(*logger.queue));
