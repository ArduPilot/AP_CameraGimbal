...
static bool sparse;
int __wrap_fstatfs64(int fd, struct statfs64 *fs)
{
    (void)fd;
...
ssize_t __real_pwrite64(int fd, const void *data, size_t size, off64_t offset);
ssize_t __wrap_pwrite64(int fd, const void *data, size_t size, off64_t offset)
{
...
int main(int argc, char **argv)
{
    ...
    while (start < size) {
        ...
        uint8_t *au = malloc(end - start + 4096); assert(au);
        ...
        if (sparse) {
            ...
            au = realloc(au, used + large); assert(au);
