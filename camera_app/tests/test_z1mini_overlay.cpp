...
#include "../src/backends/z1mini/ax_capture.cpp"
#undef ca_overlay_bitmaps
#undef dlsym
#undef main
#include <assert.h>

extern "C" int ca_overlay_bitmaps(struct ca_overlay_bitmap [CA_OVERLAY_REGIONS], unsigned, unsigned,
...
void *test_dlsym(void *handle, const char *name) noexcept
{
    (void)handle;
    if (!strcmp(name,"AX_SYS_Mmap")) return reinterpret_cast<void *>(map_pixels);
    if (!strcmp(name,"AX_SYS_Munmap")) return reinterpret_cast<void *>(unmap_pixels);
    return NULL;
}
...
{
    void *sensor[15] = {};
    sensor[10] = reinterpret_cast<void *>(sensor_flip);
