...
def run_case(args, directory, fixture, signing=True, password=True, codec='h264'):
    ...
    import keydb_lib
    user, engineer, v1, v2 = [port() for _ in range(4)]
    ...
    keydb_lib.set_video_ports(db, engineer, [v1, v2])
    for i in range(2):
...
''')
    ...
    try:
        ...
        def feed():
            try:
                while not done.is_set():
                    ...
                    fc.mav.global_position_int_send(1000, -353632610, 1491652300, 620250, 36500, 100, 200, 0, 1234)
                    fc.mav.attitude_send(1000, .1, -.2, .3, 0, 0, 0)
        ...
        wait_for(lambda: (directory / 'camera.log').read_text().count('publishing to') >= streams,
        ...
        if args.reconnect:
            ...
            connected = (directory / 'camera.log').read_text().count('publishing to')
            stop_group(proxy)
            time.sleep(3)
            ...
            proxy = subprocess.Popen([str(args.proxy / 'supportproxy')], cwd=directory,
                                     stdout=proxy_log, stderr=subprocess.STDOUT, start_new_session=True)
            wait_for(lambda: (directory / 'camera.log').read_text().count('publishing to') >= connected + streams,
