...
`CameraGimbalSITL.exe`. Select **MT11**, **A8** or **ZR10**, the mounting
orientation and either the simple video patterns or 3D terrain imagery, then
click **Start SITL**. **Open Web UI** opens the camera controls at
`http://127.0.0.1:8081/` (initial login `admin` / `ardupilot`).
...
Only one camera can use the default ports at a time. The existing environment
variables `<CAMERA>_SITL_WEB_PORT`, `_CAMERA_PORT`, `_RTSP_PORT`,
`_MAVLINK_TCP_PORT` and `_MAVLINK_UDP_PORT` override the ports when needed.
...
Stop using the launcher's **Stop** button so recordings are closed cleanly.
...
Run `dist\CameraGimbalSITL\SITLWorker.exe --self-test` to exercise all three
